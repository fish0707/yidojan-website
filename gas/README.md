...
- **只要沒被停用就會進轉盤，還沒建菜單也算**。這個轉盤的用途是「先決定去哪一間」，
  菜單是決定之後的事——新增一間店卻要先把菜單建完才能抽，本末倒置。
  轉到還沒建菜單的店時，結果卡片會附「去建這間的菜單 →」的捷徑
  （`?r=<restaurantId>&tab=menu`，會直接跳到菜單分頁並選好那間）。
- 停用（`active=FALSE`）的餐廳不會進轉盤——那是明確表示「先不要用這間」。
- 餐廳不到 5 間就有幾間放幾間；只有 1 間時直接顯示那間，不用抽。
- **這一輪沒抽到的餐廳會列在轉盤下方**，點一下就能換上轉盤。少了這塊，剛新增餐廳的人
  會以為自己沒加成功（「我有新增但沒有出現在轉盤」），其實只是這輪隨機沒抽到。
